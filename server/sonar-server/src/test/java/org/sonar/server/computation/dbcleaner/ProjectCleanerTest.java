...
public class ProjectCleanerTest {
  ...
  public void no_profiling_when_property_is_false() {
    ...
    underTest.purge(mock(DbSessionImpl.class), mock(IdUuidPair.class), settings, emptyList());
  ...
  public void call_period_cleaner_index_client_and_purge_dao() {
    ...
    verify(periodCleaner).clean(any(DbSessionImpl.class), anyString(), any(Settings.class));
    verify(dao).purge(any(DbSessionImpl.class), any(PurgeConfiguration.class), any(PurgeListener.class), any(PurgeProfiler.class));
  }

  @Test
  public void if_dao_purge_fails_it_should_not_interrupt_program_execution() {
    doThrow(RuntimeException.class).when(dao).purge(any(DbSessionImpl.class), any(PurgeConfiguration.class), any(PurgeListener.class), any(PurgeProfiler.class));
  ...
  public void if_profiler_cleaning_fails_it_should_not_interrupt_program_execution() {
    doThrow(RuntimeException.class).when(periodCleaner).clean(any(DbSessionImpl.class), anyString(), any(Settings.class));
